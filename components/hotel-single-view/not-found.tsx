import Image from "next/image";
import Link from "next/link";
import logo from '@/public/icon.png';
import watermark from '@/public/shared/Bg-watermark.png';

export default function NotFound() {
  return (
    <div className="z-50 fixed inset-0 flex justify-center items-center bg-gradient-to-b from-black via-gray-800 to-black font-poppins text-white">
      <div className="absolute inset-0 bg-cover bg-center opacity-10" style={{ backgroundImage: `url(${watermark.src})` }} />

      <div className="relative flex flex-col items-center bg-gradient-to-r from-gray-700 via-gray-800 to-gray-700 shadow-lg mx-4 p-6 border rounded-lg max-w-md text-center">
        <div className="mb-6">
          <Image src={logo} alt="Serviced Apartments Logo" width={80} height={80} priority />
        </div>

        <h1 className="font-bold text-6xl text-secondary">404</h1>
        <h2 className="mt-4 font-semibold text-slate-300 text-xl">
          Hotel Not Found
        </h2>

        <p className="mt-3 text-gray-300 text-sm">
          The hotel you’re looking for doesn’t exist or has been moved. Please check the URL or try searching again.
        </p>

        <div className="space-y-4 mt-6 w-full">
          <Link
            href="/"
            className="block bg-primary hover:bg-blue-950 shadow px-4 py-2 rounded-lg focus:ring-2 focus:ring-blue-300 w-full font-medium text-center text-white transition duration-300 focus:outline-none"
          >
            Return to Homepage
          </Link>
        </div>
      </div>
    </div>
  );
}
